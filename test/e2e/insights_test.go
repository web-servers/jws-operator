...
var _ = Describe("WebServerControllerTest", Ordered, func() {
	...
	waitForPod := func() string {
		...
		Eventually(func() bool {
			...
			if createdWebServer == nil || len(createdWebServer.Status.Pods) == 0 {
				return false
			}
			podName = createdWebServer.Status.Pods[0].Name
			return true
	...
	checkLogsNotContainString := func(podName, substring string) {
		Eventually(func() bool {
			...
			return strings.Contains(logs, substring)
		}, time.Minute*2, time.Second).Should(BeFalse(), "Log of pod %s must NOT contain '%s'", podName, substring)
	}
	...
	Context("InsightsTests", func() {
		...
		It("SimpleInsightsReportSendTest", func() {
			...
			webserver.Spec.EnvironmentVariables = []corev1.EnvVar{
				{Name: "INSIGHTS_DEBUG", Value: "true"},
			}
		...
		It("InsightsAgentPersistsAfterPodRestart", func() {
			...
			Eventually(func() bool {
				...
				currentPodName := createdWebServer.Status.Pods[0].Name
				if currentPodName != initialPodName {
		...
		// Deploys the application with 3 replicas at once.
		// The agent must start successfully and send a report in EACH of the 3 pods.
		It("MultiplePodsInDeploymentTest", func() {
			webserver.Spec.Replicas = 3
			webserver.Spec.UseInsightsClient = true
			webserver.Spec.EnvironmentVariables = []corev1.EnvVar{
				{
					Name:  "INSIGHTS_DEBUG",
			...
			Eventually(func() int {
				createdWebServer := getWebServer(name)
				if createdWebServer == nil {
					return 0
				}
				return len(createdWebServer.Status.Pods)
		...
		// Tests that the agent is not injected when the inject-agent label is missing
		It("MissingInjectAgentLabelTest", func() {
			webserver.Spec.EnvironmentVariables = []corev1.EnvVar{
				{
					Name:  "INSIGHTS_DEBUG",
					Value: "true",
			...
			podName := waitForPod()
			checkLogsNotContainString(podName, StartingInsightsAgentLog)
		})
